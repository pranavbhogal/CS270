...
(define (all_q L)
  (cond ((null? L) #t)
        ((equal? (first L) 'q)(andi #t (all_q (rest L))))
        (else #f)
  )
)
...
(define (all_q_v2 L)
  (foldr andi #t (map (lambda (x) (equal? x 'q)) L))
)
...
(define (at_least_one_q L)
  (cond ((null? L) #f)
        ((equal? (first L) 'q)(ori #t (at_least_one_q (rest L))))
        (else (ori #f (at_least_one_q (rest L))))
  )
)
...
(define (at_least_one_q_v2 L)
  (foldr ori #f (map (lambda (x) (equal? x 'q)) L))
)
...
(define (odd_amt_q L)
  (cond ((null? L) #f)
        ((equal? 'q (first L)) (xori #t (odd_amt_q (rest L))))
...
(define (odd_amt_q_v2 L)
  (foldr xori #f (map (lambda (x) (equal? x 'q)) L))
)
