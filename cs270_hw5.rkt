...
(define (lookup target environment)
  (cond
    ...
    ((equal? (first(first environment)) target) (second(first environment)))
...
(define (get-variables expression)
  (cond
    ((is-constant? expression) '())
    ((is-variable? expression) (remove-duplicates(append (list expression))))
    ((and (is-not? expression) (remove-duplicates(append(get-variables (second expression))))))
...
(define (make_bindings name)
  (list (list (list name #t)) (list(list name #f)));Implement Me
)
...
(define (insert_binding binding environments)
  (cond
   ((null? environments) '())
   (else(append(list(append binding (first environments))) (insert_binding binding (rest environments))))
  )
)
...
(define (make-truth-table var_names)
  (cond
    ((null? var_names) '())
    (else(extend_table (first var_names) (make-truth-table (rest var_names))))
  )
)
...
(define (run-on-truth-table expression tt)
  (cond
    ((null? tt) '())
    (else (append (cons (bool-eval expression (first tt)) `())(append (run-on-truth-table expression (rest tt)) `() )))
  )
)
;Tests
